...
const getNotaById = async (req: Request, res: Response) => {
  try {
    ...
    if (!nota) {
      return res
        .status(404)
        .json({ error: `No existe la nota con el ID ${id}` });
    }
  ...
  try {
    ...
    if (legajo !== undefined) {
      ...
      if (!alumnoExiste) {
        return res
          .status(400)
          .json({ error: `No existe un alumno con el legajo ${legajo}` });
      }
    }
...
const deleteNota = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(200)
      .json({ msg: `Nota con ID ${id} eliminada`, nota: eliminado });
  } catch (error) {
    console.error(error);
    return res
      .status(500)
