...
const getProfesorById = async (req: Request, res: Response) => {
    try {
        ...
        if (!profesor) {
            return res.status(404).json({ error: `No existe el profesor con el legajo ${legajo}` })
        }

        return res.status(200).json(profesor)
    } catch (error) {
        console.error(error)
        return res.status(500).json({ error: `No se pudo obtener el detalle del profesor con legajo ${req.params.legajo}` })
    }
}
...
const createProfesor = async (req: Request, res: Response) => {
    try {
        ...
        const { nombre, apellido, email, titulo, materiasDictadas, departamento, isActive } = req.body;
        ...
        if (materiasDictadas && materiasDictadas.length > 0) {
            ...
            const materiasInexistentes = materiasDictadas.filter(
                (idMateria: string) => !materias.some((m: any) => m.idMateria === idMateria)
            );
        ...
        const nuevoProfesor = new ProfesorModel(nombre, apellido, email, nuevo_legajo, titulo, materiasDictadas, departamento, isActive);
...
const updateProfesor = async (req: Request, res: Response) => {
    try {
        ...
        profesores[index] = {
            ...
            materiasDictadas: materiasDictadas !== undefined ? materiasDictadas : profesores[index].materiasDictadas,
            departamento: departamento !== undefined ? departamento : profesores[index].departamento,
        ...
        console.error(error);
        return res.status(500).json({ error: `No se pudo actualizar el profesor con legajo ${req.params.legajo}` });
    }
}
...
const deleteProfesor = async (req: Request, res: Response) => {
    try {
        ...
        await writeDB('profesores', profesores)
        return res.status(200).json({ msg: `Profesor con legajo ${legajo} eliminado`, profesor: eliminado })
    } catch (error) {
        console.error(error)
        return res.status(500).json({ error: `No se pudo eliminar el profesor con legajo ${req.params.legajo}` })
    }
}

export { getProfesorAll, getProfesorById, createProfesor, updateProfesor, deleteProfesor }